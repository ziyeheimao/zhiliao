<template>
  <div class="user">
    <el-avatar :size="45" :src="userPic" @click="avatarClick"></el-avatar>

    <ul>
      <li v-if="!Token" @click="dialogVisible = true">登录 / 注册</li>
      <li v-if="Token">个人中心</li>
      <li v-if="Token">发布纸条</li>
...
export default {
  ...
  computed: {
    ...
    userPic () {
      return this.User.userPic ? this.User.userPic : '../../../static/img/userPic.png'
    }
  },
  data () {
    return {
  ...
  methods: {
    ...
    avatarClick () {
      if (this.Token) {
        // this.$router.push('./xxx') // 跳转个人中心页面
      } else {
        this.dialogVisible = true // 登陆窗口
      }
    },
    // 按回车
    keyup (e) {
